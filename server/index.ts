...
app.get('/api/groups/:id', (c) => {
  const group = store.getGroup(c.req.param('id'));
...
app.post('/api/groups/:id/invite', (c) => {
  const id = c.req.param('id');
  if (!store.getGroup(id)) return c.json({ error: 'not found' }, 404);
...
// --- settlement proposal (mints an HMAC intent; matcher lands in Phase D) --
